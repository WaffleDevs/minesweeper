...
function placeleboard(mode, redo, id) {
    ...
    for (let i = 1; i <= blocks; i++) {
        ...
        $("body").append(
            $(
                `<div id=${i} class='block' style='height:calc(50vw/${Math.sqrt(blocks)}); width:calc(50vw/${Math.sqrt(blocks)}); max-width: ${540 / Math.sqrt(blocks)}px;  max-height: ${540 / Math.sqrt(blocks)}px;'><p id=${i + "p"
                }></p></div>`
            )
        );
    }
